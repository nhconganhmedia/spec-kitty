...
_Outcome: TypeAlias = _AlreadyReported | _Ready | None
...
def report(
    ...
    *,
    ...
    def attempt_once() -> _AlreadyReported | _Ready | None:
    ...
    # (it joins to `object` instead of `_AlreadyReported | _Ready`); attempt_once()'s own
    # signature above is the real, checked contract, so this narrows what mypy could not.
    outcome = cast(
        "_AlreadyReported | _Ready | None",
        retry_with_backoff(
            attempt_once,
    ...
    if outcome is None:
        ...
        return
    if isinstance(outcome, _AlreadyReported):
        return
    if replay is not None:
        verify_replay_checkout(root, replay)
    if dry_run:
