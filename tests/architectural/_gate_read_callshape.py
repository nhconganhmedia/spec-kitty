...
_COORD_AWARE_CALLSHAPE_RESOLVERS = _TOPOLOGY_ROUTED_READ_RESOLVERS | frozenset({"_resolve_setup_plan_feature_dir"})
...
_IDENTITY_READ_FUNCS = frozenset({"resolve_mission_identity", "get_mission_type"})
...
def _names_bound_from_primary_read_dir(func: ast.AST) -> set[str]:
    ...
    for node in ast.walk(func):
        ...
        if not isinstance(value, ast.Call) or not _is_primary_partition_read_dir_call(value):
            continue
...
def _param_position(func: ast.FunctionDef | ast.AsyncFunctionDef, name: str) -> int | None:
    return next(
        (index for index, arg in enumerate([*func.args.posonlyargs, *func.args.args]) if arg.arg == name),
        None,
    )


def _is_parameter(func: ast.FunctionDef | ast.AsyncFunctionDef, name: str) -> bool:
    return any(arg.arg == name for arg in (*func.args.posonlyargs, *func.args.args, *func.args.kwonlyargs))
...
def _caller_binds_arg_coord_aware(
    ...
    primary = _names_bound_from(caller, _PRIMARY_FOLD_CALLSHAPE_FUNCS) | (_names_bound_from_primary_read_dir(caller))
    for node in ast.walk(caller):
        ...
        if any(isinstance(arg, ast.Name) and arg.id in coord and arg.id not in primary for arg in candidates):
            return True
    return False


def _one_hop_caller_is_coord_aware(callee_func: ast.AST, param_name: str, module: ast.Module) -> bool:
    ...
    return any(caller is not callee_func and _caller_binds_arg_coord_aware(caller, callee_func.name, pos, param_name) for caller in _iter_module_functions(module))
...
def _flagged_first_arg(
    ...
    *,
    ...
    elif isinstance(first, ast.Attribute) and first.attr not in _SANCTIONED_PRIMARY_ATTRS:
...
def callshape_violations(
    ...
    *,
    ...
    primary_bound = _names_bound_from(func, _PRIMARY_FOLD_CALLSHAPE_FUNCS) | (_names_bound_from_primary_read_dir(func))
    violations: list[str] = []
    for node in ast.walk(func):
