...
class TestAttributionGuard:
    ...
    def test_blank_actor_raises_and_never_reaches_write_artifact(self, tmp_path: Path, actor: str) -> None:
        with (
            patch(
...
class TestLedgerM16ReadCallSite:
    def test_read_current_content_resolves_via_tracer_file_kind_only(self, tmp_path: Path) -> None:
        ...
        def _fake_read_dir(kind: MissionArtifactKind) -> Path:
            ...
            if kind is MissionArtifactKind.RETROSPECTIVE:
                raise AssertionError("must never resolve RETROSPECTIVE from the tracer read call site")
            return tmp_path
        ...
        with patch("specify_cli.retrospective.tracer_writer.placement_seam") as seam_ctor:
            ...
            content = tracer_writer._read_current_coord_content(tmp_path, _MISSION_SLUG, "tooling-friction.md")

        assert content == ""  # no file at tmp_path/traces/tooling-friction.md
        assert seen_kinds == [MissionArtifactKind.TRACER_FILE]

    def test_unroutable_read_degrades_to_empty_base_not_a_raise(self, tmp_path: Path) -> None:
        ...
        raise."""
        with patch("specify_cli.retrospective.tracer_writer.placement_seam") as seam_ctor:
            seam_ctor.return_value = MagicMock(read_dir=MagicMock(side_effect=ActionContextError("FEATURE_CONTEXT_UNRESOLVED", "mission does not resolve")))
            content = tracer_writer._read_current_coord_content(tmp_path, _MISSION_SLUG, "tooling-friction.md")
        assert content == ""
...
class TestRoutesThroughWriteSeamHelper:
    def test_calls_write_artifact_with_tracer_file_kind_and_staged_local_path(self, tmp_path: Path) -> None:
        ...
        with (
            patch("specify_cli.retrospective.tracer_writer.placement_seam") as seam_ctor,
            patch(
                "specify_cli.retrospective.tracer_writer.write_artifact",
        ...
        assert "files" not in captured, "tracer_writer must pass stage=, not the historical files= contract"
        ...
        assert staged_path == (tmp_path / "kitty-specs" / _MISSION_SLUG / "traces" / "design-decisions.md")
