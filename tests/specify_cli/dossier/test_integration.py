...
class TestOptionalArtifactHandling:
    ...
    def test_optional_artifacts_not_required(self, minimal_feature_dir):
        ...
        assert dossier.completeness_status == "complete", "Missing optional should not block completeness"
        ...
        try:
            ...
            for artifact in dossier.artifacts:
                if not artifact.is_present:
                    assert artifact.error_reason is not None, f"Unreadable artifact {artifact.artifact_key} missing error_reason"
        finally:
            os.chmod(unreadable, 0o644)
