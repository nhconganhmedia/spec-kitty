...
class TestResolvePathRefHitPatterns:
    ...
    def test_tactic_built_in_flat(self) -> None:
        result = _resolve_path_ref("src/charter/offering/tactics/built-in/tdd-red-green-refactor.tactic.yaml")
    ...
    def test_tactic_built_in_subdirectory(self) -> None:
        ...
        result = _resolve_path_ref("src/charter/offering/tactics/built-in/testing/acceptance-test-first.tactic.yaml")
        assert result == ("tactic", "acceptance-test-first")

    def test_paradigm_built_in(self) -> None:
        result = _resolve_path_ref("src/charter/offering/paradigms/built-in/domain-driven-design.paradigm.yaml")
    ...
    def test_directive_built_in_normalised(self) -> None:
        ...
        result = _resolve_path_ref("src/charter/offering/directives/built-in/030-test-and-typecheck-quality-gate.directive.yaml")
    ...
    def test_styleguide_built_in(self) -> None:
        result = _resolve_path_ref("src/charter/offering/styleguides/built-in/testing-principles.styleguide.yaml")
        assert result == ("styleguide", "testing-principles")

    def test_toolguide_built_in(self) -> None:
        result = _resolve_path_ref("src/charter/offering/toolguides/built-in/maven-review-checks.toolguide.yaml")
        assert result == ("toolguide", "maven-review-checks")

    def test_procedure_built_in(self) -> None:
        result = _resolve_path_ref("src/charter/offering/procedures/built-in/some-workflow.procedure.yaml")
        assert result == ("procedure", "some-workflow")

    def test_agent_profile_built_in(self) -> None:
        result = _resolve_path_ref("src/charter/offering/agent_profiles/built-in/java-jenny.agent.yaml")
    ...
    def test_paradigm_built_in_subdirectory(self) -> None:
        ...
        result = _resolve_path_ref("src/charter/offering/paradigms/built-in/category/event-sourcing.paradigm.yaml")
        assert result == ("paradigm", "event-sourcing")

    def test_toolguide_built_in_subdirectory(self) -> None:
        result = _resolve_path_ref("src/charter/offering/toolguides/built-in/ci/github-actions.toolguide.yaml")
        assert result == ("toolguide", "github-actions")

    def test_styleguide_built_in_subdirectory(self) -> None:
        result = _resolve_path_ref("src/charter/offering/styleguides/built-in/lang/python-style.styleguide.yaml")
...
class TestResolvePathRefFlattenedHome:
    ...
    def test_tactic_flattened_flat(self) -> None:
        assert _resolve_path_ref("packs/built-in/tactics/tdd-red-green-refactor.tactic.yaml") == ("tactic", "tdd-red-green-refactor")

    def test_tactic_flattened_subdirectory(self) -> None:
        assert _resolve_path_ref("packs/built-in/tactics/testing/acceptance-test-first.tactic.yaml") == ("tactic", "acceptance-test-first")

    def test_paradigm_flattened(self) -> None:
        assert _resolve_path_ref("packs/built-in/paradigms/domain-driven-design.paradigm.yaml") == ("paradigm", "domain-driven-design")

    def test_directive_flattened(self) -> None:
        result = _resolve_path_ref("packs/built-in/directives/030-test-and-typecheck-quality-gate.directive.yaml")
        assert result is not None
        kind, raw_id = result
        assert kind == "directive"
        assert raw_id.startswith("030-")
    ...
    def test_styleguide_flattened(self) -> None:
        assert _resolve_path_ref("packs/built-in/styleguides/testing-principles.styleguide.yaml") == ("styleguide", "testing-principles")

    def test_toolguide_flattened(self) -> None:
        assert _resolve_path_ref("packs/built-in/toolguides/maven-review-checks.toolguide.yaml") == ("toolguide", "maven-review-checks")

    def test_procedure_flattened(self) -> None:
        assert _resolve_path_ref("packs/built-in/procedures/some-workflow.procedure.yaml") == ("procedure", "some-workflow")

    def test_agent_profile_flattened(self) -> None:
        assert _resolve_path_ref("packs/built-in/agent_profiles/java-jenny.agent.yaml") == ("agent_profile", "java-jenny")
    ...
    def test_flattened_home_never_reintroduces_inner_built_in(self) -> None:
        ...
        assert _resolve_path_ref("packs/built-in/tactics/built-in/tdd-red-green-refactor.tactic.yaml") == ("tactic", "tdd-red-green-refactor")
...
class TestResolvePathRefMissPatterns:
    ...
    def test_proposed_agent_profile_returns_none(self) -> None:
        ...
        assert _resolve_path_ref("src/charter/offering/agent_profiles/_proposed/python-implementer.agent.yaml") is None

    def test_architecture_adr_returns_none(self) -> None:
        assert _resolve_path_ref("docs/adr/3.x/2026-04-14-2-agent-skills-renderer.md") is None
...
class TestStyleguideWalkEdges:
    ...
    def test_aggregate_design_rules_suggests_paradigm(self) -> None:
        ...
        edge_targets = {e.target for e in edges if e.source == "styleguide:aggregate-design-rules" and e.relation == Relation.SUGGESTS}
    ...
    def test_testing_principles_suggests_directive_030(self) -> None:
        ...
        nodes, edges = extract_artifact_edges(DOCTRINE_ROOT)
        edge_targets = {e.target for e in edges if e.source == "styleguide:testing-principles" and e.relation == Relation.SUGGESTS}
    ...
    def test_test_desiderata_suggests_testing_principles(self) -> None:
        ...
        nodes, edges = extract_artifact_edges(DOCTRINE_ROOT)
        edge_targets = {e.target for e in edges if e.source == "styleguide:test-desiderata-and-boundaries" and e.relation == Relation.SUGGESTS}
    ...
    def test_planning_and_tracking_suggests_github_tracker(self) -> None:
        ...
        nodes, edges = extract_artifact_edges(DOCTRINE_ROOT)
        edge_targets = {e.target for e in edges if e.source == "styleguide:planning-and-tracking" and e.relation == Relation.SUGGESTS}
    ...
    def test_java_conventions_suggests_maven_review_checks(self) -> None:
        ...
        nodes, edges = extract_artifact_edges(DOCTRINE_ROOT)
        edge_targets = {e.target for e in edges if e.source == "styleguide:java-conventions" and e.relation == Relation.SUGGESTS}
    ...
    def test_no_duplicate_styleguide_suggests_edges(self) -> None:
        ...
        sg_triples = [(e.source, e.target, e.relation.value) for e in edges if e.source.startswith("styleguide:") and e.relation == Relation.SUGGESTS]
    ...
    def test_styleguide_suggests_edges_have_valid_targets(self) -> None:
        ...
        for edge in edges:
            if edge.source.startswith("styleguide:") and edge.relation == Relation.SUGGESTS:
                assert edge.target in node_urns, f"Dangling styleguide suggests target: {edge.target}"

    def test_non_doctrine_refs_produce_no_edges(self) -> None:
        """URLs and non-doctrine paths must not produce any DRG edges."""
        _, edges = extract_artifact_edges(DOCTRINE_ROOT)
        url_edges = [e for e in edges if e.source.startswith("styleguide:") and (e.target.startswith("http") or e.target.startswith("glossary:"))]
        assert url_edges == [], f"Unexpected edges from non-doctrine references: {url_edges}"
...
class TestGraphWithStyleguideEdges:
    ...
    def test_generate_graph_with_styleguide_edges_is_valid(self, tmp_path: Path) -> None:
    ...
    def test_styleguide_suggests_edges_present_in_generated_graph(self, tmp_path: Path) -> None:
        """Generated graph must contain at least some styleguide suggests edges."""
        output = tmp_path / "graph.yaml"
        graph = generate_graph(DOCTRINE_ROOT, output)
        sg_suggests = [e for e in graph.edges if e.source.startswith("styleguide:") and e.relation == Relation.SUGGESTS]
        assert len(sg_suggests) > 0, "No styleguide suggests edges in generated graph; styleguide walk may be broken"

    def test_graph_edge_count_gte_before_styleguide_walk(self, tmp_path: Path) -> None:
        ...
        assert len(graph.edges) > 561, f"Edge count ({len(graph.edges)}) is not greater than the pre-WP08 baseline of 561; styleguide edges may not have been added"
    ...
    def test_shipped_graph_is_fresh(self, tmp_path: Path) -> None:
        ...
        assert regenerated == committed, (
            "The committed built-in DRG source is stale after WP08 styleguide walk (DD-11 per-file byte-identity). Regenerate: spec-kitty doctrine regenerate-graph"
        )
