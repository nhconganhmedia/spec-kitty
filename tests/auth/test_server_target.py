...
def test_all_fields_populated_under_env_equals_config(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_corrupt_config_toml_is_treated_as_no_configured_url(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    (target_root / "config.toml").write_text("this is = = not valid toml", encoding="utf-8")
...
def test_blank_config_server_url_defers_to_env(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    (target_root / "config.toml").write_text('[sync]\nserver_url = ""\n', encoding="utf-8")
    monkeypatch.setenv(SAAS_URL_ENV_VAR, ENV_URL)
    target = resolve_server_target()
    assert target.configured_server_url is None
...
def test_non_table_sync_key_is_treated_as_no_configured_url(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    (target_root / "config.toml").write_text('sync = "oops"\n', encoding="utf-8")
    monkeypatch.setenv(SAAS_URL_ENV_VAR, ENV_URL)
    target = resolve_server_target()
    assert target.configured_server_url is None
...
def test_env_equals_config_is_not_an_override(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _write_config(target_root, CONFIG_URL)
...
def test_missing_config_with_env_resolves_to_normalized_env_url(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_missing_config_with_differing_env_is_process_override(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(SAAS_URL_ENV_VAR, ENV_URL)
    target = resolve_server_target()
    assert target.configured_server_url is None
    assert target.override_mode is OverrideMode.PROCESS_OVERRIDE
...
def test_config_set_with_differing_env_and_process_override_env_wins(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _write_config(target_root, CONFIG_URL)
...
def test_ambiguous_setup_only_disagreement_fails_closed(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _write_config(target_root, CONFIG_URL)
    monkeypatch.setenv(SAAS_URL_ENV_VAR, ENV_URL)
...
def test_env_equals_config_never_raises_even_with_process_wide_override_false(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _write_config(target_root, CONFIG_URL)
...
def test_env_only_machine_resolves_cleanly_even_setup_only(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_process_override_of_configured_target_logs_warning(target_root: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    _write_config(target_root, CONFIG_URL)
    monkeypatch.setenv(SAAS_URL_ENV_VAR, ENV_URL)  # disagrees with config
...
def test_env_only_machine_does_not_log_warning(target_root: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
...
def test_env_equals_config_does_not_log_warning(target_root: Path, monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
...
def test_env_naming_the_packaged_default_is_a_real_opinion_and_wins(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_env_naming_the_packaged_default_with_no_config_resolves_to_default(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_explicit_canonical_env_overrides_stale_retired_saved_target(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_explicit_canonical_env_vs_stale_retired_saved_target_fails_closed_setup_only(target_root: Path, monkeypatch: pytest.MonkeyPatch) -> None:
