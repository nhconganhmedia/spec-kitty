...
class TestGetPacksRootDefault:
    ...
    def test_env_override_round_trips_through_default(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestGetRuntimeStateRoot:
    ...
    def test_env_override_used_verbatim(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        override = tmp_path / "custom-state-root"
        monkeypatch.setenv("SPEC_KITTY_HOME", str(override))
        assert get_runtime_state_root() == override

    def test_posix_default_is_dot_spec_kitty_under_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("SPEC_KITTY_HOME", raising=False)
        monkeypatch.setattr("kernel.paths.is_windows", lambda: False)
        assert get_runtime_state_root() == Path.home() / ".spec-kitty"

    def test_distinct_from_kittify_home_by_directory_name(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_windows_uses_platformdirs_non_roaming(self, monkeypatch: pytest.MonkeyPatch) -> None:
        import platformdirs
    ...
    def test_pure_no_directory_created(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
